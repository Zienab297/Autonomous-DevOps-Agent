"""
repositories/solution_repo.py
------------------------------
Repository for Solution persistence.

Changes vs original:
- All timestamps use datetime.now(timezone.utc)
- Structured logging includes incident_id + source
- Sensitive content (healing_prompt) is never logged verbatim
"""
...
class SolutionRepository:
    ...
    def create(self, solution_obj, project_id: str, incident_id: str) -> SolutionModel:
        ...
        self._s.flush()
        logger.info(
            "[DB][solution_repo] created solution id=%s incident_id=%s source=%s confidence=%s",
            row.id, incident_id, row.source, row.confidence,
        )
        return row
    ...
    def list_by_project(
        ...
        stmt = select(SolutionModel).where(SolutionModel.project_id == project_id)
        if incident_id:
            stmt = stmt.where(SolutionModel.incident_id == incident_id)
        stmt = stmt.order_by(SolutionModel.confidence.desc().nullslast())
